...
class DoctorManage extends Component {
    ...
    componentDidMount() {
        ...
        this.props.handleGetAllSpecialist(100);
    }
    ...
    componentDidUpdate(prevProps, prevState, snapshot) {
        ...
        if (this.props.getDoctorDetailByIdResponse !== prevProps.getDoctorDetailByIdResponse) {
            ...
            if (this.props.getDoctorDetailByIdResponse.data?.Doctor_infor) {
                let { priceId, paymentId, provinceId, clinicId, specialistId } =
                    this.props.getDoctorDetailByIdResponse.data.Doctor_infor;
                ...
                if (this.state.clinicList?.length > 0) {
                    let found = this.state.clinicList.find((item) => item.id === clinicId);
                    this.setState({ clinic: found });
                }

                if (this.state.specialistList?.length > 0) {
                    let found = this.state.specialistList.find((item) => item.id === specialistId);
                    this.setState({ specialist: found });
                }
            }
            // console.log(this.state.specialistList);
        ...
        if (this.props.getAllSpecialistRes !== prevProps.getAllSpecialistRes) {
            // console.log(this.props.getAllSpecialistRes);
            if (this.props.getAllSpecialistRes?.data && this.props.getAllSpecialistRes?.data.length > 0) {
                this.setState({
                    specialistList: this.props.getAllSpecialistRes.data.map((item) => {
        ...
        if (this.props.getAllClinicRes !== prevProps.getAllClinicRes) {
            ...
            if (this.props.getAllClinicRes?.data && this.props.getAllClinicRes?.data.length > 0) {
                this.setState({
                    clinicList: this.props.getAllClinicRes.data.map((item) => {
                        return { value: item.id, label: item.name, ...item };
                    }),
                });
            }
        ...
        return (
            <div className="doctor-manage p-2">
                ...
                <div className="container">
                    ...
                    <div className="row my-2">
                        <div className="doctor-specialist col">
                            <label for="" className="">
                                <FormattedMessage id="system.doctor-manage.specialist" />
                            </label>
                            <Select
                                ...
                                options={this.state.specialistList}
                            />
                        </div>
                        <div className="doctor-specialist col">
                            <label for="" className="">
                    ...
                    <div
                        ...
                        onClick={() => {
                            console.log(this.state.specialist?.id);
...
const mapDispatchToProps = (dispatch) => {
    return {
        ...
        handleGetAllSpecialist: (limit) => dispatch(actions.handleGetAllSpecialist(limit)),
        handleGetAllClinic: (limit) => dispatch(actions.handleGetAllClinic(limit)),
    };
};
